...
const TrackListPagination = ({
  ...
  for (let x = 0; x < pages; x++) {
    if (x === currentPage) {
      p.push(
        <span
          style={{ color: '#C8C7C7', cursor: 'pointer' }}
          onClick={() => setPage(x)}>
          •
        </span>
      )
      ...
      p.push(
        <span style={{ cursor: 'pointer' }} onClick={() => setPage(x)}>
          •
        </span>
      )
    }
  ...
  return (
    <>
      <div
        ...
        <p style={{ flexShrink: 0, color: '#B5B5B5', marginBottom: '0px' }}>
          {index}.
        </p>
        <p
          style={{
          ...
          <span style={{ color: '#E9E9E9', overflowX: 'hidden' }}>{title}</span>{' '}
          — {author}
        </p>
...
const TrackList = ({ media, setMedia }: TrackListProps) => {
  ...
  const deleteTrack = (num: number) =>
    setMedia({
      ...media,
      tracks: media.tracks
        .slice(0, num)
        .concat(media.tracks.slice(num + 1, media.tracks.length))
    })
  ...
  useEffect(() => {
    setItems(
      media.tracks
        ...
        .map((track: PlaylistTrack, index: number) => (
          <TrackListItem
            key={`track-list-item-${track.title}-${track.author}`}
            title={track.title}
            author={track.author}
            index={index + 1 + pageNum * 10}
  ...
  return (
    <>
      <p
        ...
        {hidden ? 'Open track list' : 'Close track list'}
      </p>
